...
function absUrl(storeUrl, u) {
  u = String(u || '');
  return u.startsWith('/uploads/') ? storeUrl + u : u;
}
...
function newProductsHtml(items, storeUrl) {
  const cards = items.slice(0, 6).map((p) => `
    <div style="border:1px solid #eee;border-radius:12px;padding:12px;margin:8px 0">
      <img src="${escHtml(absUrl(storeUrl, p.image))}" width="120" style="border-radius:8px"/><br/>
      <b>${escHtml(p.title)}</b><br/>$${escHtml(p.price)}
      <br/><a href="${escHtml(productLink(storeUrl, p))}">شاهد التفاصيل</a> |
      <a href="${escHtml(storeUrl)}/go/${encodeURIComponent(p.id)}" target="_blank">اشترِ الآن</a>
    </div>`).join('');
...
function buildSitemap(storeUrl, items, articles) {
  const urls = ['', '/sitemap.xml', '/robots.txt', '/blog',
    ...items.map((p) => `/p/${encodeURIComponent(p.id)}`),
